...
public class Notch {
    ...
    public static NotchParams getInfo(Context ctx) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.P) {
            return null;
        }

        NotchParams info = new NotchParams();
        ...
        if (r != 0) {
            ...
            if (gravity != Gravity.CENTER && Math.abs(bounds.centerX() - metrics.widthPixels / 2f) <= AndroidUtilities.dp(2)) {
                gravity = Gravity.CENTER;
            }
            ...
            info.isLikelyCircle = bounds.width() <= AndroidUtilities.dp(32) || bounds.width() <= bounds.height();
    ...
    public final static class NotchParams {
        public int gravity;
