...
public class VoIPNotificationsLayout extends LinearLayout {
    ...
    public void addNotification(String text, String tag, boolean animated) {
        ...
        if (isAnyVideo) {
            background = Theme.createRoundRectDrawable(AndroidUtilities.dp(12), 0x64000000);
        } else {
            background = new RoundedBackgroundDrawable(Color.BLACK, 12, 90);
        }
    ...
    public void setIsVideo(boolean isAnyVideo) {
        if (this.isAnyVideo == isAnyVideo)
            return;

        this.isAnyVideo = isAnyVideo;
        for (int i = 0; i < getChildCount(); i++) {
            View v = getChildAt(i);
