...
public class DeleteEffect extends TextureView {
    public static String TAG = "DeleteEffect";
    private AnimationThread thread;

    public DeleteEffect(@NonNull Context context) {
        super(context);
        init();
    }

    public DeleteEffect(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public DeleteEffect(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private static class AnimationThread extends Thread {
        ...
        private void loop() {
            synchronized (lock) {
                ...
                while (running) {
                    ...
                    lastTime = now;
                    Log.i(TAG, "Delta time=" + Δt);

                    if (Δt < MIN_DELTA && !isFirstFrame) {
                        ...
                        Δt = MIN_DELTA;
                    } else if (Δt > MAX_DELTA && isAdjustmentPhase) {
                        double adjustedForGeneration = Δt - lastGenerationTime;
        ...
        private void init() {
            ...
            GLES31.glShaderSource(vertexShader, RLottieDrawable.readRes(null, R.raw.delete_vertex) + "\n// " + Math.random());
            ...
            GLES31.glShaderSource(fragmentShader, RLottieDrawable.readRes(null, R.raw.delete_fragment) + "\n// " + Math.random());
        ...
        private int initVertex(
            ...
            // Position
            vertices[index++] = (x / (float)width - 0.5f) * 2f;
            vertices[index++] = (y / (float)height - 0.5f) * -2f;
            // Texture
            vertices[index++] = 0f;
            vertices[index++] = 0f;
            // Velocity
