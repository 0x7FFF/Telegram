...
public class ProfileAvatarImageView extends BackupImageView {
    ...
    private float inset;
    private DrawHelper drawHelper = canvas -> {
        ImageReceiver imageReceiver = animatedEmojiDrawable != null ? animatedEmojiDrawable.getImageReceiver() : this.imageReceiver;
        float alpha = 1.0f;
        int height = (int) (getMeasuredHeight() - AndroidUtilities.dp(64 + 12 * 2) * blurProgress / ((View) getParent()).getScaleY());
    ...
    public ProfileAvatarImageView(Context context) {
        ...
        blurHelper = new ProfileAvatarBlurHelper(this) {
            ...
            protected float getBottomOffset() {
                return getMeasuredHeight() - AndroidUtilities.dp(64 + 12 * 2 + 32) / getScaleY() * blurProgress;
            }
        };
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        path.rewind();
        float hOff = scaleY != 0 ? AndroidUtilities.dp(64 + 12 * 2 + 32) / scaleY * progressToExpand : 0;
        ...
        blurHelper.draw(drawHelper, canvas);
        canvas.restore();
    }
